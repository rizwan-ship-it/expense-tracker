...
import {
  addExpense,
  updateExpense
} from '../redux/features/expense/expenseListSlice'
import { useNavigate, useParams } from 'react-router-dom'

function EditExpensePage () {
  const expenseId = useParams().id
  const expenseList = useSelector(state => state.expense.expenseList)
  const expDetails = expenseList.find(ele => ele.id === expenseId)
  ...
  const handleExpenseSubmitform = e => {
    ...
    dispatch(updateExpense({ expensePayload }))
      ...
      <div className='flex flex-col  justify-center items-center h-screen bg-gray-200'>
        <div className='border-2 flex flex-col gap-4 w-150 border-slate-400 p-5 rounded-xl bg-gray-100 inset-shadow-sm inset-shadow-indigo-500/50'>
          ...
          <div className='flex flex-col  '>
            ...
            <input
              className='outline-none border-2 border-slate-300 rounded px-3 h-9 text-gray-800 '
              type='text'
              name='title'
          ...
          <div className='flex flex-col '>
            <label className='font-medium ' id='amount'>
              Amount:
            </label>
            <input
